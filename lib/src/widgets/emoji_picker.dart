...
class EmojiWrapper {
  const EmojiWrapper({required this.emoji, required this.category});
...
class _CustomEmojiPickerState extends State<CustomEmojiPicker> {
  ...
  void _populateEmojiRows() {
    for (var categoryEmoji in defaultEmojiSet) {
      ...
      for (var emoji in categoryEmoji.emoji) {
        row.add(EmojiWrapper(emoji: emoji, category: categoryEmoji.category));
  ...
  void _handleCategoryIconClick(CategoryEmoji categoryEmoji, double emojiSize) {
    _scrollController.jumpTo(categoryOffsets[categoryEmoji.category]!);
    setState(() => selectedCategory = categoryEmoji.category);
  }
  ...
  Widget build(BuildContext context) {
    emojiFontSize =
        (MediaQuery.of(context).size.width - 2 * listPadding) /
        (columnCount + 5);
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (emojiWrapperRow == null) {
                      return Container(
                        ...
                        height: headerHeight,
                        child: Text(
                          categoryNames[allEmojiRows[index + 1]!
                              .first
                              .category]!,
                          style: TextStyle(
                            color: colorTheme.onSurface,
              ...
              if (defaultTargetPlatform.isIOS) const SizedBox(height: 24),
            ],
          ),
        );
      },
      config: Config(columns: columnCount, emojiSizeMax: emojiFontSize),
    );
  }
}
